...
def parse_create_table(sql: str) -> Dict[str, Table]:
    ...
    # Diccionario de valores comunes de DEFAULT
    default_values_dict = {
        'GETDATE()': 'GETDATE()',
        ...
        if line.lower().startswith('create table'):
            ...
            if len(column_info) >= 2:
                ...
                if 'NOT NULL' in line.upper():
                    # Mover NOT NULL al campo "Default"
                    default_value = 'NOT NULL'
                if 'CHECK' in line.upper():
                    col_type.append('CHECK')

                # Manejo del valor DEFAULT
                default_match = re.search(r'DEFAULT\s+(.+?)(?:\s+|$)', line, re.IGNORECASE)
                if default_match:
                    found_default = clean_value(default_match.group(1))
                    if found_default.upper() in default_values_dict:
                        default_value = default_values_dict[found_default.upper()]
                    else:
                        default_value = found_default
